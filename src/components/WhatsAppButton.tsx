  ...
  if (variant === "floating") {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={handleClick}
        className={`fixed bottom-6 right-6 z-[9999] w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-[#25D366] to-[#20BD5A] text-white shadow-lg hover:shadow-2xl flex items-center justify-center group relative overflow-hidden ${className}`}
  ...
  if (variant === "ghost") {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={`inline-flex items-center justify-center gap-2 h-9 px-3 rounded-full text-sm font-medium text-foreground hover:text-primary transition-all duration-300 relative overflow-hidden group shadow-md hover:shadow-lg ${className}`}
      >
        <motion.span
